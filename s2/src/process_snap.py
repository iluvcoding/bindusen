...
def l1CTol2A(dir_raw, yyyymmdd,  dir_l1c, dir_nc, l2a_process_path, pr_status):
    ...
    for zip_file in zip_files:
        ...
        ncFiles = glob.glob('%s/*/*%s*' %  (dir_nc, tileNo_time))
        n_ncFiles = len(ncFiles)
        if n_ncFiles < 35:
...
#     zip_files = glob.glob(dir_raw + '/S2?_MSIL1C_%s*T43PFP*.zip' % yyyymmdd)
#     for zip_file in zip_files:
#         tileNo_time = '%s_%s' %(os.path.basename(zip_file).split('_')[5], os.path.basename(zip_file).split('_')[2])
#         try:
...
#                 #break
#         except KeyError:
#             print('Tile %s not of interest' %tileNo_time)
# =============================================================================
            
            
def extractL2A(dir_raw, yyyymmdd,  dir_l2a, l2a_process_path, pr_status):
    zip_files = glob.glob(dir_raw + '/S2?_MSIL2A_%s*.zip' % yyyymmdd)
    for zip_file in zip_files:
        tileNo_time = '%s_%s' %(os.path.basename(zip_file).split('_')[5], os.path.basename(zip_file).split('_')[2])
        try:
            if not pr_status[tileNo_time]:
                safe_file = extract_download_data(zip_file, dir_l2a)
                #break
        except KeyError:
            print('Tile %s not of interest', tileNo_time)